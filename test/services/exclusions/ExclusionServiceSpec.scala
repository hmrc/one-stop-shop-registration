...
class ExclusionServiceSpec extends BaseSpec with BeforeAndAfterEach {
  ...
  ".findExcludedTrader" - {
    ...
    "must return ExcludedTrader if vrn is matched" in {
      ...
      when(mockConfig.excludedTraders) thenReturn Seq(HashedExcludedTrader(hashedVrn, exclusionCode, exclusionReason, exclusionPeriod))

      val expected = ExcludedTrader(vrn, exclusionCode, exclusionReason, exclusionPeriod)
