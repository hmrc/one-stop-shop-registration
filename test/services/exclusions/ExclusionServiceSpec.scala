...
class ExclusionServiceSpec extends BaseSpec with BeforeAndAfterEach {
  ...
  private val exclusionService = new ExclusionService(mockConfig)
  ...
  private val hashedVrn = exclusionService.crypto.hash(PlainText("123456789"))
  ...
  ".findExcludedTrader" - {
    ...
    "must return ExcludedTrader if vrn is matched" in {
      ...
      when(mockConfig.excludedTraders) thenReturn Seq(HashedExcludedTrader(hashedVrn.value, exclusionSource, exclusionReason, exclusionPeriod))
